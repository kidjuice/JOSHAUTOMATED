...
(function () {
  var pairs = {
    'xauusd': { price: 2341.50, spread: 0.8,  decimals: 2 },
    'eurusd': { price: 1.0847,  spread: 0.0002, decimals: 4 },
    'nas100': { price: 18234.2, spread: 5,    decimals: 1 },
    'gbpusd': { price: 1.2714,  spread: 0.0003, decimals: 4 },
    'usdjpy': { price: 151.82,  spread: 0.02, decimals: 2 },
    'sp500':  { price: 5218.4,  spread: 2,    decimals: 1 },
    'btcusd': { price: 67420,   spread: 20,   decimals: 0 },
    'us30':   { price: 38941,   spread: 3,    decimals: 0 }
  };

  var bases = {};
  for (var k in pairs) bases[k] = pairs[k].price;

  function updateTicker() {
    for (var key in pairs) {
      var info  = pairs[key];
      var delta = (Math.random() - 0.495) * info.spread;
      info.price = Math.max(0.0001, info.price + delta);

      var priceEl  = document.getElementById('t-'  + key);
      var changeEl = document.getElementById('tc-' + key);
      if (!priceEl || !changeEl) continue;

      var formatted = info.price.toFixed(info.decimals);
      if (info.decimals === 0) formatted = Math.round(info.price).toLocaleString();

      var pct    = ((info.price - bases[key]) / bases[key]) * 100;
      var isUp   = pct >= 0;
      ...
      /* flash the price element */
      priceEl.style.color = isUp ? '#00d68f' : '#ff4d6a';
      setTimeout(function (el) { el.style.color = ''; }, 400, priceEl);
    }
  }
...
var allReveal = document.querySelectorAll('.rv, .rv-left, .rv-right, .rv-scale');
...
/* ── MARK PRO CARD AS FEATURED ─────────────── */
(function () {
  var cards = document.querySelectorAll('.bot-card');
  cards.forEach(function (c) {
    if (c.querySelector('.bot-name') &&
        c.querySelector('.bot-name').textContent.trim() === 'Pro') {
...
var userRating = 0;
var reviews    = [];
try { reviews = JSON.parse(localStorage.getItem('joshstb_reviews') || '[]'); } catch (e) {}

var starPicks = document.querySelectorAll('.star-pick');
starPicks.forEach(function (s) {
  s.addEventListener('mouseover', function () {
    var v = parseInt(s.dataset.v, 10);
    starPicks.forEach(function (x, i) { x.classList.toggle('lit', i < v); });
  });
  s.addEventListener('click', function () {
    userRating = parseInt(s.dataset.v, 10);
    starPicks.forEach(function (x, i) { x.classList.toggle('lit', i < userRating); });
  });
});
...
function submitReview() {
  ...
  var review = {
    id: Date.now(), name: name, tier: tier, country: country,
    text: text, rating: userRating,
